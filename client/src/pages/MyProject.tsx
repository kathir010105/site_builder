...
const MyProject = () => {
  ...
  return (
    <>
      <div className="px-4 md:pc-16 lg:px-24 xl:px-32">
        {loading ? (
            ...
            <div className="flex flex-wrap gap-3.5">
              {projects.map((project) => (
                <div onClick={()=>navigate(`/projects/${project.id}`)}
                  key={project.id}
                  className="relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden showdow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300"
                >
                  ...
                  <div className="p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors">
                    <div className="flec items-start justify-between">
                    ...
                    <p className="text-gray-400 mt-1 text-sm line-clamp-2e">
                      {project.initial_prompt}
                    </p>

                    <div
                      onClick={(e) => e.stopPropagation()}
                      className="flex justify-between items-centermt-6"
                    >
          ...
          <div className="flex flex-col items-center justify-center h-[80vh]">
            ...
            <button
              onClick={() => navigate("/")}
              className="text-white pc-5 py-2 met-5 rounded-md bg-indigo-500 hover:bg-indigo-600 active:scale-95 transition-all"
            >
              Create New
            </button>
          </div>
